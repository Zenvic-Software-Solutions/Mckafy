...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $checkQuery = "SELECT id FROM food_tbl WHERE food_name = '$food_name' AND id != '$id'";
    $checkResult = mysqli_query($conn, $checkQuery);
    if (mysqli_num_rows($checkResult) > 0) {
        echo json_encode(["status" => "error", "message" => "Product name already in use!"]);
        exit;
    }

    // Fetch existing image path before updating
    $oldImageQuery = "SELECT food_image FROM food_tbl WHERE id = '$id'";
    ...
    if (mysqli_query($conn, $query)) {
        echo json_encode(["status" => "success", "message" => "Food item updated successfully!", "image" => $imagePath]);
    } else {
